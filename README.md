...
| `@loncra/pro`（以后） | IM / Agent UI，依赖 client + antdv |

管理端：`"@loncra/client": "file:../packages/client"`、`"@loncra/antdv": "file:../packages/antdv"`。
...
当前控件：`TooltipValidationFormItem`、`QrCodeModal`、`BasicImage`、`IconSelect`、`EmojiButton`、`Markdown` / `MarkdownCodeRenderer`、`KeyValueTable`、`Editor`（`antdv-next-tiptap`）。后续按同样链路逐个搬无 Service 控件。要对 API 的上传 / IM 进 `@loncra/antdv-pro`。
